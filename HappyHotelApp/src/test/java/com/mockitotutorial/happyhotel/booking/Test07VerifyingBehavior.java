...
class Test07VerifyingBehavior {
	...
	void should_InvokePayment_When_Prepaid() {
		...
		// when
		//we simply know the method was invoked,but we don't know what happened behind the scenes.
		//we want to verify the paymentService mock was called.
		//In other words, we want to make sure that paymentService.pay() was executed correctly in makeBooking method.
		bookingService.makeBooking(bookingRequest);
		// then
		//On this line,we check whether the method pay() from the paymentServiceMock was called with these specific argument.
//		verify(paymentServiceMock).pay(bookingRequest, 400.0); //price 輸入500會fail
		verify(paymentServiceMock,times(1)).pay(bookingRequest, 400.0);
		//this Mockito method checks if any other methods form this mock were called. If paymentServiceMock is called
